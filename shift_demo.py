...
# Scenario varsayılan anahtarlar
...
if MENU=='Veriler':
    ...
    ship=MGR['scenario']['ship_hour']; early_days=MGR['scenario']['early_days']
    if scen=='erken':
    ...
    if st.button('Kaydet Senaryo'):
        MGR['scenario']={'type':scen,'ask_ara':ask,'ship_hour':ship,'early_days':early_days}
        save_db(DB)
        st.success('Kaydedildi')
    st.divider(); st.header('Çalışanlar')
    with st.expander('Yeni Çalışan Ekle'):
        c1,c2=st.columns(2)
        nm=c1.text_input('İsim'); sc=c2.text_input('Sicil')
        ...
        if st.button('Ekle') and nm and sc:
            MGR['employees'].append({'name':nm,'sicil':sc,'pt':pt,'pt_days':pt_days,'ht_day':ht})
...
if MENU=='Vardiya Oluştur':
    ...
    iz=st.session_state.setdefault('iz',{})
    with st.expander('İzin / Rapor'):
        emp=st.selectbox('Çalışan',['—']+[e['name'] for e in MGR['employees']])
        day=st.selectbox('Gün',DAYS); typ=st.selectbox('Tür',['Rapor','Yıllık İzin'])
        if st.button('Ekle İzin') and emp!='—':
            iz[emp]={'day':day,'type':('Rapor' if typ=='Rapor' else 'Yİ')}
            st.success('Eklendi')
    denkl=st.session_state.get('denkl',None)
    with st.expander('Denkleştirme'):
        ...
        if use:
            ...
            d_day=st.selectbox('Fazla Çalışılacak Gün',DAYS,key='dday')
            exit_d=st.selectbox('Erken Çıkış Günü',['']+DAYS,key='dexit')
    ...
    if st.button('Vardiya Oluştur 🛠️'):
        ...
        st.session_state['iz']={}; st.download_button("Excel'e Aktar",pretty.to_csv(index=False).encode('utf-8-sig'))
